...
export default function BusConsumptionDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 py-8">
      ...
      {/* Header Info */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm mb-8 w-full">
        <div className="p-3 bg-white shadow-sm rounded border">
          <p className="text-gray-500">Department</p>
          <p className="font-medium text-gray-800">
            {bus.issueBill?.department || "-"}
          </p>
        </div>
        <div className="p-3 bg-white shadow-sm rounded border">
          <p className="text-gray-500">Issued By</p>
          <p className="font-medium text-gray-800">{bus.issueBill?.issuedBy || "-"}</p>
        </div>
        <div className="p-3 bg-white shadow-sm rounded border">
          <p className="text-gray-500">Date</p>
          <p className="font-medium text-gray-800">
            {bus.issueBill?.issueDate
              ? new Date(bus.issueBill.issueDate).toLocaleDateString("en-IN")
              : "-"}
          </p>
        </div>
      </div>
      ...
      {/* Items Table */}
      <h3 className="text-xl font-semibold text-gray-700 mb-3">Items Consumed</h3>
            ...
            <tbody>
              {bus.issueBill.items.map((it, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 border">{it.code || it.item?.code || "-"}</td>
                  <td className="px-3 py-2 border">
                    {it.description || it.item?.headDescription || ""}
                  </td>
                  <td className="px-3 py-2 border">{it.uqc || it.item?.unit || "-"}</td>
                  <td className="px-3 py-2 border text-right">{it.quantity}</td>
                  <td className="px-3 py-2 border text-right">₹{it.amount || 0}</td>
                </tr>
              ))}
              {/* Summary Row */}
              <tr className="font-semibold bg-gray-100">
                ...
                <td className="px-3 py-2 border text-right">
                  {bus.issueBill.items.reduce((sum, it) => sum + (it.quantity || 0), 0)}
                </td>
                <td className="px-3 py-2 border text-right">
                  ₹{bus.issueBill.totalAmount || 0}
                </td>
              </tr>
            </tbody>
          </table>
