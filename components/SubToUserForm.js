...
export default function SubToUserForm() {
    ...
    const fetchData = async () => {
      try {
        const [itemsRes, summaryRes, userRes, busesRes] = await Promise.all([
          API.get("/items"), // includes lastPurchaseRate
  ...
  // ✅ Handle item or bus change (auto-fill rate + auto-calc amount)
  ...
  const addRow = () =>
    setItems([...items, { item: "", quantity: "", rate: "", amount: "", busId: "" }]);
    ...
    const payload = {
      ...
      items: items.map((i) => {
        ...
        return {
          ...
          bus: selectedBus ? selectedBus._id : null,
        };
      }),
    };
  ...
  return (
    <div>
      ...
      <div className="max-w-full mx-auto mt-8 bg-white shadow-md p-6 rounded-lg border border-gray-200 relative">
        ...
        {/* Header */}
        <h2 className="text-3xl font-bold mb-4 text-gray-800">Sub → User</h2>
